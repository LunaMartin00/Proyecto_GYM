...
-- 1. CONFIGURACIÓN DE AUDITORIA
 
-- Para que esto funcione, deben crear manualmente una carpeta llamada "Auditoria" 
-- en su disco C. Si no, dará error.

-- Verificar si ya existe para no duplicar
IF NOT EXISTS (SELECT * FROM sys.server_audits WHERE name = 'Audit_OlympusGym_Server')
BEGIN
    CREATE SERVER AUDIT [Audit_OlympusGym_Server]
    TO FILE 
...
-- Habilitar la auditoría a nivel servidor
ALTER SERVER AUDIT [Audit_OlympusGym_Server] WITH (STATE = ON);
GO
...
-- 2. DIMENSIONAMIENTO TÉCNICO 

-- Consulta administrativa para visualizar el tamaño real de las tablas y filas.
-- Sirve como evidencia del cálculo de dimensionamiento.

SELECT 
    t.name AS [Nombre Tabla],
    p.rows AS [Cantidad Filas],
    CAST(ROUND(((SUM(a.total_pages) * 8) / 1024.00), 2) AS NUMERIC(36, 2)) AS [Espacio Total (MB)],
    CAST(ROUND(((SUM(a.used_pages) * 8) / 1024.00), 2) AS NUMERIC(36, 2)) AS [Datos Usados (MB)],
    CAST(ROUND(((SUM(a.total_pages) - SUM(a.used_pages)) * 8) / 1024.00, 2) AS NUMERIC(36, 2)) AS [Espacio Libre (MB)]
FROM 
    sys.tables t
    INNER JOIN      
